<template>
  <div>
      ...
      <div class="col-p12 card page-body" v-if="!noEmployee">
            ...
            <div class="col-12 lg:col-4 xl:col-4">
              <div class="p-field">
                <label
                  ...
                  for="ochBaseId"
                  :class="{
                    'p-error': validationErrors.ochBaseId && submitted,
                  }"
                >
                  Base
                </label>
                <Dropdown
                  id="ochBaseId"
                  :filter="true"
                  v-model="selectedOchBase"
                  @change="setOchBase()"
                  :options="bases"
                  optionLabel="name"
                  :class="{
                    'p-invalid': validationErrors.bases && submitted,
                  }"
                />
              </div>
            </div>
        ...
        <div class="p-fluid">
          <div class="grid">
            <div class="col-12 lg:col-12 xl:col-12">
              <div class="p-field">
                <label for="object">{{ $t("VOUCHERS.OBJECT") }}</label>
                <InputText
                  id="object"
                  v-model="och.object"
                  @input="validate()"
        ...
        <div id="budget_line_grid">
          ...
          >
            ...
            <Column field="description" header="Item" style="width: 20%">
              <template #body="e">
                <div class="link">
    ...
    <h6>4. {{ $t("PRF.ADVANCE_JUSTICATION") }}</h6>
         <div class="p-fluid">
        <div class="grid">
          <div class="col-12 lg:col-4 xl:col-4">
              ...
              <div class="p-field">
                ...
                <Calendar
                  ...
                  v-model="prf.justification.date"
                  :showButtonBar="true"
                  :maxDate="new Date()"
                  :showIcon="true"
                  placeholder="dd/mm/yyyy"
          ...
          <div class="col-12 lg:col-4 xl:col-4">
            <div class="p-field">
                <label for="justification_amount">{{
                ...
                <InputNumber
                  ...
                  :locale="enterprise.currency_intel_number_format"
                  :minFractionDigits="2"
                  v-model="prf.justification.amount"
                  style="width: 100%"
                />
              </div>
          </div>
          <div class="col-12 lg:col-4 xl:col-4">
                        <div class="p-field">
                <label for="justification_amount_returned">{{
                  $t("VOUCHERS.AMOUNT_RETURNED")
                }}</label>
                <InputNumber
                  id="justification_amount_returned"
                  :currency="enterprise.currency_format_key"
                  :locale="enterprise.currency_intel_number_format"
                  :minFractionDigits="2"
                  v-model="prf.justification.amount_returned"
                  style="width: 100%"
                />
              </div>
          </div>
...
import OCHService from "./och.service";
...
export default defineComponent({
  ...
  methods: {
    init() {
      ...
      EnterpriseService.read()
        ...
        .then((projects) => {
          ...
          return OCHService.base.read();
        })
        .then((bases) => {
          this.bases = bases.map((m) => {
    ...
    setOchBase() {
      this.och.och_base_id = this.selectedOchBase.id;
      this.validate();
    },
    setAccount() {
      this.och.account_to_credit = this.selectedAccount.uuid;
      this.validate();
    },
    getUserInformations() {
    ...
    viewPDF(voucherUuid) {
      OCHService.read(voucherUuid).then((och) => {
        this.voucherUuid = voucherUuid;
        this.fileName = this.createFileName(och);
    ...
    loadVoucherInfo(voucherUuid) {
      ...
      OCHService.read(voucherUuid)
        .then((result) => {
          ...
          const pMethods = this.bases.filter(
            (method) => method.id === result.och.och_base_id
          );

          this.selectedOchBase = pMethods[0] || {};
          this.och.och_base_id = result.och.och_base_id;
          this.och.object = result.och.object;
          this.prf.usefulness = result.prf.usefulness;
          this.prf.additional_info = result.prf.additional_info;

          this.prf.justification = {
            date : new Date(result.och.justif_date), 
            amount: result.och.justif_amount, 
            amount_returned : result.och.justif_amount_returned
          }
    ...
    validate(showErrors) {
      ...
      if (!isFutureDate) {
        validator.check("delivery_expected_date", true, this.validationErrors);
      }
      ...
      validator.check("object", !this.och.object);
      validator.check(
        "grid",
        incorretLines !== 0 || specifiedLines === 0 || this.total > 500
      );
      validator.check(
        "update_note",
        this.voucherUuid && !this.clone ? !this.prf.update.note : false
      );
      ...
      if (this.total > 500) {
        NotifyService.danger(this, "FORM.ERRORS.INVALID", "VOUCHERS.OCH_NOTE");
        return;
      }
    ...
    submit() {
      ...
      OCHService.create({
        ...
        employee: this.applicant, // demandeur
        parent: !this.clone ? this.voucherUuid : null, // doc previous version
        ...
        voucher_type_id: constants.voucherTypes.och,
        ...
        justification: {
          date : UtilService.formatDate(this.prf.justification.date),
          amount : this.prf.justification.amount,
          amount_returned: this.prf.justification.amount_returned,
        },

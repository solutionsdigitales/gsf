<template>
  <div class="col-p12 card page-body" style="padding-bottom: 100px">
      ...
      <Column field="action" header="Action" style="max-width: 5rem">
        <template #body="e">
          <PRFActions
            ...
            v-on:reloadVoucherList="getOCHs()"
          />
        </template>
      </Column>
    ...
    <div>{{ voucherList.length }} {{ $t("FORM.INFO.ROWS") }}</div>
...
export default defineComponent({
  ...
  created() {
    ...
    this.filters = OCHservice.filters;
  ...
  methods: {
    viewPDF(och) {
      this.voucherUuid = och.uuid;
      this.fileName = OCHservice.getFileName(och);
    ...
    downloadExcel() {
      ...
      params.voucher_type_id = constants.voucherTypes.ddf;
      const getParams = OCHservice.serelize(params);
      const uri = `${this.server}ochs/download/excel?lang=${this.lang}&${getParams}`;
    ...
    getOCHs() {
      ...
      OCHservice.read(null, params)
        .then((prfs) => {
          this.voucherList = prfs;
          this.totals = 0;
          prfs.forEach((p) => {

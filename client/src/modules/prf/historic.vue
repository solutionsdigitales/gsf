    ...
    <div class="grid">
      <div class="col-12">
        <div>
          <h4>
            {{ $t("PRF.HISTORIC") }} :
            <span class="text-primary">{{ prfReference }}</span>
          </h4>
          ...
          >
            ...
            <template #content="slotProps">
              <Card>
                ...
                <template #content>
                  <span class="legend">{{ $t(slotProps.item.legend) }}</span
                  ><br />
                  <span
                    ><b>{{ slotProps.item.employee_name }}</b></span
                  ><br />
                  <span
                    class="link text-primary prf"
                    v-if="slotProps.item.viewPDF"
                    @click="viewPDF(slotProps.item)"
                  >
...
export default defineComponent({
  ...
  methods: {
    getPrfs() {
      PRFService.historic(this.prfReference).then((result) => {
          ...
          if (item.type === "APPROVE") {
            ...
            item.legend =
              item.type === "NOTE"
                ? "FORM.LEGENDS.NOTE"
                : item.version === 1
                ? "FORM.LEGENDS.CREATION"
                : "FORM.LEGENDS.EDITION";
            item.color =
              item.type === "NOTE"
                ? "rgb(96, 125, 139)"
                : item.version === 1
                ...
                : "rgb(103, 58, 183)";
            item.icon =
              item.version === 1 ? "pi pi-circle-fill" : "pi pi pi-pencil";
            item.viewPDF = true;
          }
    ...
    formatDate(date) {
      if (!date) return;
      return util.formatDate(date, "DD/MM/YYYY H:mm:s");
    },
    viewPDF(document) {
      const lang = this.$i18n.locale;
      this.server = this.$store.state.server;
      const docUrl = DocumentService.url(document.voucher_type_id);
    ...
    pdfDownloadFunction() {
      const urls = `/${this.info.docUrl}`;
      const target = `export/${this.info.document.uuid}?lang=${this.info.lang}&renderer=pdf&stream=1`;
      console.log(urls, target);
      return DownloadService.pdf(urls, target, "file.pdf", true);
    },
    closePrfViewer() {

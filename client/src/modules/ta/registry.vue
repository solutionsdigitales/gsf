      ...
      <Column field="action" header="Action" style="max-width: 5rem">
        <template #body="e">
          <TAActions
            ...
            v-on:reloadPRFList="getTravalAdvances()"
          />
        </template>
      </Column>
...
import TAActions from "./actions.vue";
...
export default defineComponent({
  ...
  created() {
    ...
    this.filters = TravalAdvanceService.filters;
  ...
  methods: {
    viewPDF(ta) {
      this.voucherUuid = ta.uuid;
      this.fileName = TravalAdvanceService.getFileName(ta);
    ...
    downloadExcel() {
      ...
      params.voucher_type_id = constants.voucherTypes.ta;
      const getParams = TravalAdvanceService.serelize(params);
      const uri = `${this.server}traval_advances/download/excel?lang=${this.lang}&${getParams}`;
    ...
    getTravalAdvances() {
      ...
      TravalAdvanceService.read(null, params)
        .then((prfs) => {
          this.prfList = prfs;
          this.totals = 0;
          prfs.forEach((p) => {
          ...
          this.getTravalAdvancestatus();
        })
        .finally(() => {
          this.loading = false;
        });
    },
    getTravalAdvancestatus() {
